...
{
    public static function activacion($global)
    {
        if (password_verify($_SESSION["usuario"]["correo"], $_REQUEST["id"])) {
            BaseDatos::validarCorreo();
            return true;
        } else {
            return false;
        }
    ...
    public static function enviarCorreo($receptor, $nombre)
    {
        //Creamos el contenido del asunto 
        $subject = "Verificación de correo del cine";
        //Cuerpo del mensaje
        $message = 'Hola ' . $nombre . ',<br> Para verificarse pulsa en este link: <a href=http://localhost/dwes/hacer/cinev1/index.php?peticion=correo&id='.password_hash($receptor, PASSWORD_DEFAULT).'>Verificacion</a> <br> 
        Enviado el día: ' . date("Y-m-d H:i:s");
        ...
        try {
            ...
            // $mail->addAttachment('imagenes/imagen.png');
